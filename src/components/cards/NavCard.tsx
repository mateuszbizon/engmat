...
function NavCard({ item, isScrolled }: NavCardProps) {
    ...
    <>
        {item.isLink ? (
            <NavigationMenuLink asChild>
                <Button className={`py-0 px-6 ${isActive ? "text-primary" : isScrolled ? "text-foreground" : "text-white"} hover:text-primary text-lg`} variant={"link"} asChild>
                    <Link href={item.href}>
                        {item.label}
                    </Link>
                </Button>
